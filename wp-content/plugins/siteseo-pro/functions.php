...
function siteseo_pro_load_license($parent = 0){
	...
	// Load license
	$siteseo->license = get_option($license_field, []);
	
	// Update license details as well
	if(!empty($siteseo->license) && !empty($siteseo->license['license']) && (time() - @$siteseo->license['last_update']) >= 86400){
		
		$resp = wp_remote_get($license_api_url.'license.php?license='.$siteseo->license['license'].'&url='.rawurlencode(site_url()));
	...
	if(empty($siteseo->license) || empty($siteseo->license['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			if(!empty($softaculous_license['license']) && 
				(!empty($softaculous_license['active']) || empty($siteseo->license['license']))
			){
				$siteseo->license = $softaculous_license;
			}
		}elseif(empty($parent)){
			$siteseo->license = get_option('softaculous_pro_license', []);
			
			if(!empty($siteseo->license)){
				siteseo_pro_load_license(1);
			}
		}
	}
