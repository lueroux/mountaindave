...
class License{
	...
	static function save(){
		...
		$resp = wp_remote_get(SITESEO_API . 'license.php?license=' . $license, array('timeout' => 30));
		...
		if(is_wp_error($resp) || 200 !== wp_remote_retrieve_response_code($resp)){
			if(is_wp_error($resp)){
				...
				echo '<div style="margin-top:65px;" class="notice notice-error is-dismissible"><p>';
				echo esc_html__('An error occurred, please try again. Response code: ', 'siteseo-pro') . wp_remote_retrieve_response_code($resp);
				echo '</p></div>';
				return;
			}
		} else {
			$siteseo->license = json_decode(wp_remote_retrieve_body($resp), true);
			 if(empty($siteseo->license)){
				echo '<div style="margin-top:65px;" class="notice notice-error is-dismissible"><p>';
			...
			$siteseo->license = $siteseo->license;
			update_option('siteseo_license', $siteseo->license);
