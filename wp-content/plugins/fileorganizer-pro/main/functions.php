...
function fileorganizer_pro_load_license($parent = 0){
	...
	// Load license
	$fileorganizer->license = get_option($license_field, array());
	
	// Update license details as well
	if(!empty($fileorganizer->license) && !empty($fileorganizer->license['license']) && (time() - @$fileorganizer->license['last_update']) >= 86400){
		
		$resp = wp_remote_get($license_api_url.'license.php?license='.$fileorganizer->license['license'].'&url='.rawurlencode(site_url()));
	...
	if(empty($fileorganizer->license) || empty($fileorganizer->license['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			if(!empty($softaculous_license['license']) && 
				(!empty($softaculous_license['active']) || empty($fileorganizer->license['license']))
			){
				$fileorganizer->license = $softaculous_license;
			}
		}elseif(empty($parent)){
			$fileorganizer->license = get_option('softaculous_pro_license', []);
			
			if(!empty($fileorganizer->license)){
